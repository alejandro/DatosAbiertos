...
describe('Feeds', function() {
	...
	describe('when adding a collection to a feed', function() {
		it('should add the collection in the database', function(done) {
			feedModule.addCollection(userId, feed2._id, 'collection name', 'collectionCode').then(function(modifiedFeed) {
				database.collection('feeds').then(function(col) {
					col.getById(modifiedFeed._id).then(function(feedFromDatabase) {
						feedFromDatabase.collections[0].name.should.equal('collection name');
						feedFromDatabase.collections[0].code.should.equal('collectionCode');
	...
	describe('when adding a field to a collection', function() {
		it('should add the field in the database', function(whenFinished) {
			var rules = [{
				code: 'something'
			}];
	...
	describe('when changing the code of a collection in a feed', function() {
		it('should change the collection code in the db', function(done) {
			var theCollection = feed3.collections[0];
			var feedId = feed3._id;
			var collectionId = theCollection._id;
			feedModule.changeCollectionCode(userId, feedId, collectionId, 'newCode').then(function(modifiedFeed) {
				database.collection('feeds').then(function(col) {
					col.getById(modifiedFeed._id).then(function(feedFromDatabase) {
						feedFromDatabase.collections[0].code.should.equal('newCode');
					}).done(done);
				});
			});
		});
